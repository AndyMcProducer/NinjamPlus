...
(async()=>{
 ...
 for(const file of ['live-video-probe.js','live-source-observer.js','live-scheduler-observer.js'])await context.addInitScript({content:await fs.readFile(path.join(__dirname,file),'utf8')});
 ...
 const deadline=Date.now()+600000;await event('ready');
 while(Date.now()<deadline){
   try{const command=JSON.parse(await fs.readFile(path.join(output,'command.json'),'utf8'));await fs.unlink(path.join(output,'command.json'));

...
ArrayXXc phase_vocoder(const ArrayXXc& D, Real rate,
    ...
    // Instead of updating each bin's phase independently, we identify local
    // spectral peaks and lock the phases of bins within ±2 bins of each peak
    // to the peak's phase. This dramatically reduces the metallic/phased
    // artifact on transients (drum hits) that basic phase vocoders produce.
    //
    // For each output frame we:
    // 1. Compute the per-bin phase advance as normal
    // 2. Find local magnitude peaks in the interpolated spectrum
    // 3. For bins belonging to a peak's region, use the peak's phase advance
    //    instead of their own — this keeps nearby bins phase-coherent

    constexpr int lockRange = 2; // ±2 bins around each peak
    ...
    for (Eigen::Index t = 0; t < time_steps; ++t) {
        ...
        // Write output using accumulated phase
        for (Eigen::Index f = 0; f < n_freq; ++f) {
        ...
        if (t < time_steps - 1) {
            ...
            // Identity phase locking: find local peaks in the magnitude spectrum
            // and lock nearby bins' phase advances to the peak bin's advance.
            ArrayXr lockedDphi = dphi; // start with per-bin advances

            for (Eigen::Index f = 0; f < n_freq; ++f) {
                // Check if this bin is a local peak (higher than neighbors)
                bool isPeak = true;
                for (int d = -lockRange; d <= lockRange; ++d) {
                    if (d == 0) continue;
                    Eigen::Index nf = f + d;
                    if (nf < 0 || nf >= n_freq) continue;
                    if (mag(nf) > mag(f)) {
                        isPeak = false;
                        break;
                    }
                ...
                if (isPeak) {
                    // Lock bins within ±lockRange to this peak's phase advance
                    for (int d = -lockRange; d <= lockRange; ++d) {
                        Eigen::Index nf = f + d;
                        if (nf < 0 || nf >= n_freq) continue;
                        lockedDphi(nf) = dphi(f);
                    }
                }
            }

            // Update phase accumulator using locked phase advances
            phase_acc += phi_advance + lockedDphi;
        }
    }
